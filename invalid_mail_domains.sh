...
stdin=$(</dev/stdin)
extract=$(grep "status=deferred" <<< "$stdin" | grep -o "[[:space:]]to=[^[:space:]]*[[:space:]]" | sed -e "s/^[[:space:]]*to=<//" -e "s/>,[[:space:]]*$//"| sort | uniq | sed -e "s/^[^@]*@//"| sort | uniq) # Extract deferred domain names.
IFS=$'\n' # Split on newline only.
set -f    # Disable globbing.
dominio=($(printf "%s" "$extract"))
for fgdn_domain in "${dominio[@]}"; do
	(dig @${dns[$RANDOM % ${#dns[@]} ]} $fgdn_domain MX | grep MX | grep -v "^;" &>/dev/null) || (nmap -Pn -p 25 $fgdn_domain 2>/dev/null | grep "tcp open" &>/dev/null || false) # If MX records are not found, port 25 of the domain is scanned.
	if [ $? -eq 1 ]; then
        	echo "$fgdn_domain"
	fi
done 
